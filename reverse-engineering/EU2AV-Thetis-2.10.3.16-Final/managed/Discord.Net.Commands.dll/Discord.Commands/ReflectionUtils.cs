...
internal static class ReflectionUtils
{
	...
	internal static Func<IServiceProvider, T> CreateBuilder<T>(TypeInfo typeInfo, CommandService commands)
	{
		...
		return delegate(IServiceProvider services)
		{
			...
			{
				array[i] = GetMember(commands, services, parameters[i].ParameterType, typeInfo, parameters[i].GetCustomAttributes());
			}
			T val = InvokeConstructor<T>(constructor, array, typeInfo);
			PropertyInfo[] array2 = properties;
			foreach (PropertyInfo propertyInfo in array2)
			{
				propertyInfo.SetValue(val, GetMember(commands, services, propertyInfo.PropertyType, typeInfo, null));
			}
			return val;
		};
	}
