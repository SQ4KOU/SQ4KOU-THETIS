...
public class UserTypeReader<T> : TypeReader where T : class, IUser
{
	public override async Task<TypeReaderResult> ReadAsync(ICommandContext context, string input, IServiceProvider services)
	{
		...
		await channelUsers.Where((IUser x) => string.Equals(input, x.GlobalName, StringComparison.OrdinalIgnoreCase)).ForEachAsync(delegate(IUser channelUser)
		{
			AddResult(results, channelUser as T, (channelUser.GlobalName == input) ? 0.65f : 0.55f);
		}).ConfigureAwait(continueOnCapturedContext: false);
		foreach (IGuildUser item in guildUsers.Where((IGuildUser x) => string.Equals(input, x.GlobalName, StringComparison.OrdinalIgnoreCase)))
		{
			AddResult(results, item as T, (item.GlobalName == input) ? 0.6f : 0.5f);
		}
		...
		foreach (IGuildUser item2 in guildUsers.Where((IGuildUser x) => string.Equals(input, x.Username, StringComparison.OrdinalIgnoreCase)))
		{
			AddResult(results, item2 as T, (item2.Username == input) ? 0.6f : 0.5f);
		}
		...
		foreach (IGuildUser item3 in guildUsers.Where((IGuildUser x) => string.Equals(input, x.Nickname, StringComparison.OrdinalIgnoreCase)))
		{
			AddResult(results, item3 as T, (item3.Nickname == input) ? 0.6f : 0.5f);
		}
		if (results.Count > 0)
		{
