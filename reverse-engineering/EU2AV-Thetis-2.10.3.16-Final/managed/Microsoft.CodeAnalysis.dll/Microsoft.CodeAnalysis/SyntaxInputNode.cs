...
internal sealed class SyntaxInputNode<T> : SyntaxInputNode, IIncrementalGeneratorNode<T>
{
	...
	{
		return new SyntaxInputNode<T>(_inputNode, _registerOutput, comparer, _name);
	}

	public IIncrementalGeneratorNode<T> WithTrackingName(string name)
	{
		return new SyntaxInputNode<T>(_inputNode, _registerOutput, _comparer, name);
	}
	...
	internal override ISyntaxInputBuilder GetBuilder(StateTableStore table, bool trackIncrementalSteps)
	{
		return _inputNode.GetBuilder(table, this, trackIncrementalSteps, _name, _comparer);
	}
}
