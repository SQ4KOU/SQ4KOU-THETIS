...
internal readonly struct Rune : IComparable, IComparable<Rune>, IEquatable<Rune>
{
	...
	public Rune(char ch)
	{
		if ((uint)ch >= 55296u && (uint)ch <= 57343u)
		{
	...
	{
		int length = EncodeToUtf16(buffer);
			...
			while (true)
			{
				if (num3 > 0)
				{
					...
					if ((uint)num < (uint)source.Length)
					{
						...
						result = DecodeFromUtf8(source, out result2, out bytesConsumed2);
	...
	public int EncodeToUtf16(Span<char> destination)
	{
		...
		{
			new ArgumentException("Destination is too short.", "destination");
		}
		return charsWritten;
	}
	...
	public bool Equals(Rune other, StringComparison comparisonType)
	{
		...
		ReadOnlySpan<char> span = AsSpan(buffer);
		buffer = stackalloc char[2];
		ReadOnlySpan<char> other2 = other.AsSpan(buffer);
		return span.Equals(other2, comparisonType);
	}
	...
	public override string ToString()
	{
		...
		Span<char> span = stackalloc char[2];
		UnicodeUtility.GetUtf16SurrogatesFromSupplementaryPlaneScalar(_value, out span[0], out span[1]);
