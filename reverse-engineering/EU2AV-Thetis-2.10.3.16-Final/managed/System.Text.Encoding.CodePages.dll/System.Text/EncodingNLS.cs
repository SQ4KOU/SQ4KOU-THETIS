...
internal abstract class EncodingNLS : Encoding
{
	...
	public override int WindowsCodePage => System.Text.EncodingTable.GetCodePageItem(CodePage).FamilyCodePage;

	public override bool IsBrowserDisplay => (System.Text.EncodingTable.GetCodePageItem(CodePage).CodePageFlags & 2) != 0;

	public override bool IsBrowserSave => (System.Text.EncodingTable.GetCodePageItem(CodePage).CodePageFlags & 8) != 0;

	public override bool IsMailNewsDisplay => (System.Text.EncodingTable.GetCodePageItem(CodePage).CodePageFlags & 1) != 0;

	public override bool IsMailNewsSave => (System.Text.EncodingTable.GetCodePageItem(CodePage).CodePageFlags & 4) != 0;
	...
	{
		ExceptionPolyfills.ThrowIfNull(chars, "chars");
		if (index < 0 || count < 0)
		{
	...
	{
		ExceptionPolyfills.ThrowIfNull(s, "s");
		fixed (char* chars = s)
		{
	...
	{
		ExceptionPolyfills.ThrowIfNull(s, "s");
		ExceptionPolyfills.ThrowIfNull(bytes, "bytes");
		...
		fixed (char* ptr = s)
		{
			...
			{
				return GetBytes(ptr + charIndex, charCount, nonNullPinnableReference + byteIndex, byteCount, null);
			}
		}
	}
	...
	public unsafe override int GetChars(byte[] bytes, int byteIndex, int byteCount, char[] chars, int charIndex)
	{
		...
		fixed (byte* ptr = &bytes[0])
		{
			...
			{
				return GetChars(ptr + byteIndex, byteCount, nonNullPinnableReference + charIndex, charCount, null);
			}
		}
	}
