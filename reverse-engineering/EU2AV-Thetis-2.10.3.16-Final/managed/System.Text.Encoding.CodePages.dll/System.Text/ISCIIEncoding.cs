...
internal sealed class ISCIIEncoding : System.Text.EncodingNLS, ISerializable
{
	...
	private static ReadOnlySpan<int> UnicodeToIndicChar => new ReadOnlySpan<int>(new int[1135]
	{
	...
	private static ReadOnlySpan<int> IndicMappingIndex => new ReadOnlySpan<int>(new int[12]
	{
		-1, -1, 0, 1, 2, 3, 1, 4, 5, 6,
		7, 8
	});
	...
	public unsafe override int GetBytes(char* chars, int charCount, byte* bytes, int byteCount, System.Text.EncoderNLS baseEncoder)
	{
		...
		{
			if (encodingByteBuffer.AddByte(239, (byte)(_defaultCodePage | 0x40)))
			{
