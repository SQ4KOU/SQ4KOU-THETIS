...
internal class RequestBucket
{
	...
	public async Task<Stream> SendAsync(RestRequest request)
	{
		...
		while (true)
		{
			...
			try
			{
				response = await request.SendAsync().ConfigureAwait(continueOnCapturedContext: false);
				...
				if (response.StatusCode < HttpStatusCode.OK || response.StatusCode >= HttpStatusCode.MultipleChoices)
				{
					...
					throw new HttpException(response.StatusCode, request, discordError?.Code, discordError?.Message, (discordError != null && discordError.Errors.IsSpecified) ? discordError.Errors.Value.Select((ErrorDetails x) => new DiscordJsonError(x.Name.GetValueOrDefault("root"), x.Errors.Select((Error y) => new DiscordError(y.Code, y.Message)).ToArray())).ToArray() : null);
				}
			...
			{
				UpdateRateLimit(id, request, info, response.StatusCode == (HttpStatusCode)429, redirected: false, response.Stream);
			}
		}
