...
public class RestSlashCommand : RestCommandBase, ISlashCommandInteraction, IApplicationCommandInteraction, IDiscordInteraction, ISnowflakeEntity, IEntity<ulong>
{
	...
	internal override async Task UpdateAsync(DiscordRestClient client, Interaction model, bool doApiCall)
	{
		...
		Data = await RestSlashCommandData.CreateAsync(client, model2, base.Guild, model.GuildId.ToNullable(), base.Channel, doApiCall).ConfigureAwait(continueOnCapturedContext: false);
	}
}
