...
internal static class MessageHelper
{
	...
	public static Task<Message> ModifyAsync(ulong channelId, ulong msgId, BaseDiscordClient client, Action<MessageProperties> func, RequestOptions options)
	{
		...
		if (messageProperties.AllowedMentions.IsSpecified)
		{
			...
			{
				if (value2.AllowedTypes.Value.HasFlag(AllowedMentionTypes.Users))
				{
					List<ulong> userIds = value2.UserIds;
					if (userIds != null && userIds.Count > 0)
					{
						throw new ArgumentException("The Users flag is mutually exclusive with the list of User Ids.", "allowedMentions");
					}
				}
				if (value2.AllowedTypes.Value.HasFlag(AllowedMentionTypes.Roles))
				{
					List<ulong> userIds = value2.RoleIds;
					if (userIds != null && userIds.Count > 0)
					{
		...
		if (!messageProperties.Attachments.IsSpecified)
		{
			...
			return client.ApiClient.ModifyMessageAsync(channelId, msgId, args, options);
		}
		UploadFileParams obj2 = new UploadFileParams(messageProperties.Attachments.Value?.ToArray() ?? Array.Empty<FileAttachment>())
		{
			Content = messageProperties.Content
		};
		array = list?.ToArray();
		...
		obj2.MessageComponent = (messageProperties.Components.IsSpecified ? ((Optional<IMessageComponent[]>)(messageProperties.Components.Value?.Components.Select((IMessageComponent x) => x.ToModel()).ToArray() ?? Array.Empty<IMessageComponent>())) : Optional<IMessageComponent[]>.Unspecified);
		UploadFileParams args2 = obj2;
		return client.ApiClient.ModifyMessageAsync(channelId, msgId, args2, options);
	}
	...
	{
		return client.ApiClient.DeleteMessageAsync(channelId, msgId, options);
	}
	...
	{
		return client.ApiClient.AddReactionAsync(channelId, messageId, (emote is Emote emote2) ? $"{emote2.Name}:{emote2.Id}" : UrlEncode(emote.Name), options);
	}

	public static Task AddReactionAsync(IMessage msg, IEmote emote, BaseDiscordClient client, RequestOptions options)
	{
		return client.ApiClient.AddReactionAsync(msg.Channel.Id, msg.Id, (emote is Emote emote2) ? $"{emote2.Name}:{emote2.Id}" : UrlEncode(emote.Name), options);
	}
	...
	{
		return client.ApiClient.RemoveReactionAsync(channelId, messageId, userId, (emote is Emote emote2) ? $"{emote2.Name}:{emote2.Id}" : UrlEncode(emote.Name), options);
	}
	...
	{
		return client.ApiClient.RemoveReactionAsync(msg.Channel.Id, msg.Id, userId, (emote is Emote emote2) ? $"{emote2.Name}:{emote2.Id}" : UrlEncode(emote.Name), options);
	}
	...
	{
		return client.ApiClient.RemoveAllReactionsAsync(channelId, messageId, options);
	}

	public static Task RemoveAllReactionsAsync(IMessage msg, BaseDiscordClient client, RequestOptions options)
	{
		return client.ApiClient.RemoveAllReactionsAsync(msg.Channel.Id, msg.Id, options);
	}
	...
	{
		return client.ApiClient.RemoveAllReactionsForEmoteAsync(channelId, messageId, (emote is Emote emote2) ? $"{emote2.Name}:{emote2.Id}" : UrlEncode(emote.Name), options);
	}
	...
	{
		return client.ApiClient.RemoveAllReactionsForEmoteAsync(msg.Channel.Id, msg.Id, (emote is Emote emote2) ? $"{emote2.Name}:{emote2.Id}" : UrlEncode(emote.Name), options);
	}

	public static IAsyncEnumerable<IReadOnlyCollection<IUser>> GetReactionUsersAsync(IMessage msg, IEmote emote, int? limit, BaseDiscordClient client, ReactionType reactionType, RequestOptions options)
	{
		...
		return new PagedAsyncEnumerable<IUser>(100, async delegate(PageInfo info, CancellationToken ct)
		{
			...
			return (await client.ApiClient.GetReactionUsersAsync(msg.Channel.Id, msg.Id, emoji, getReactionUsersParams, reactionType, options).ConfigureAwait(continueOnCapturedContext: false)).Select((User x) => RestUser.Create(client, x)).ToImmutableArray();
	...
	{
		return client.ApiClient.RemovePinAsync(msg.Channel.Id, msg.Id, options);
	}
	...
	{
		ImmutableArray<ITag>.Builder builder = ImmutableArray.CreateBuilder<ITag>();
		int index = 0;
		int codeIndex = 0;
		while (true)
		{
	...
	{
		return client.ApiClient.CrosspostAsync(channelId, msgId, options);
	}
	...
	public static IAsyncEnumerable<IReadOnlyCollection<IUser>> GetPollAnswerVotersAsync(ulong channelId, ulong msgId, ulong? afterId, uint answerId, int? limit, BaseDiscordClient client, RequestOptions options)
	{
		return new PagedAsyncEnumerable<IUser>(100, async (PageInfo info, CancellationToken ct) => (await client.ApiClient.GetPollAnswerVotersAsync(channelId, msgId, answerId, info.PageSize, info.Position, options).ConfigureAwait(continueOnCapturedContext: false)).Users.Select((User x) => RestUser.Create(client, x)).ToImmutableArray(), delegate(PageInfo info, IReadOnlyCollection<IUser> lastPage)
		{
			if (lastPage.Count != 100)
			{
				return false;
	...
	{
		return client.ApiClient.ExpirePollAsync(channelId, messageId, options);
	}
}
