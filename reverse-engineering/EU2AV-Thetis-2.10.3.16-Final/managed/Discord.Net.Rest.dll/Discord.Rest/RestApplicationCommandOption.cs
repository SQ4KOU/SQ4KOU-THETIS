...
public class RestApplicationCommandOption : IApplicationCommandOption
{
	...
	internal void Update(ApplicationCommandOption model)
	{
		...
		Options = (model.Options.IsSpecified ? model.Options.Value.Select(Create).ToImmutableArray() : ImmutableArray.Create<RestApplicationCommandOption>());
		Choices = (model.Choices.IsSpecified ? model.Choices.Value.Select((ApplicationCommandOptionChoice x) => new RestApplicationCommandChoice(x)).ToImmutableArray() : ImmutableArray.Create<RestApplicationCommandChoice>());
		ChannelTypes = (model.ChannelTypes.IsSpecified ? ((IEnumerable<ChannelType>)model.ChannelTypes.Value).ToImmutableArray() : ImmutableArray.Create<ChannelType>());
