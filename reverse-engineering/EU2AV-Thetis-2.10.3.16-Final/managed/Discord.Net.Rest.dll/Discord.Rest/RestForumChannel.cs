...
public class RestForumChannel : RestGuildChannel, IForumChannel, IMentionable, INestedChannel, IGuildChannel, IChannel, ISnowflakeEntity, IEntity<ulong>, IDeletable, IIntegrationChannel
{
	...
	internal new static RestForumChannel Create(BaseDiscordClient discord, IGuild guild, Channel model)
	{
		RestForumChannel restForumChannel = new RestForumChannel(discord, guild, model.Id, guild?.Id ?? model.GuildId.Value);
		restForumChannel.Update(model);
		return restForumChannel;
	}
	...
	{
		Update(await ForumHelper.ModifyAsync(this, base.Discord, func, options));
	}
