...
public class RestMessageCommand : RestCommandBase, IMessageCommandInteraction, IApplicationCommandInteraction, IDiscordInteraction, ISnowflakeEntity, IEntity<ulong>
{
	...
	internal override async Task UpdateAsync(DiscordRestClient client, Interaction model, bool doApiCall)
	{
		...
		Data = await RestMessageCommandData.CreateAsync(client, model2, base.Guild, model.GuildId.ToNullable(), base.Channel, doApiCall).ConfigureAwait(continueOnCapturedContext: false);
	}
}
