...
public class RestGuildChannel : RestChannel, IGuildChannel, IChannel, ISnowflakeEntity, IEntity<ulong>, IDeletable
{
	...
	internal static RestGuildChannel Create(BaseDiscordClient discord, IGuild guild, Channel model)
	{
		...
		case ChannelType.Media:
			return RestMediaChannel.Create(discord, guild, model);
		...
		default:
			return new RestGuildChannel(discord, guild, model.Id, guild?.Id ?? model.GuildId.Value);
		}
	}
	...
	internal override void Update(Channel model)
	{
		...
		if (model.PermissionOverwrites.IsSpecified)
		{
			...
			ImmutableArray<Overwrite>.Builder builder = ImmutableArray.CreateBuilder<Overwrite>(value.Length);
