...
internal static class EntityExtensions
{
	...
	public static Emote ToEmote(this Discord.API.Emoji model, BaseDiscordClient discord = null)
	{
		return new Emote(model.Id.GetValueOrDefault(), model.Name, model.Animated.GetValueOrDefault(defaultValue: false), model.User.IsSpecified ? RestUser.Create(discord, model.User.Value) : null);
	}

	public static GuildEmote ToEntity(this Discord.API.Emoji model)
	{
		return new GuildEmote(model.Id.Value, model.Name, model.Animated.GetValueOrDefault(defaultValue: false), model.Managed.GetValueOrDefault(defaultValue: false), model.RequireColons.GetValueOrDefault(defaultValue: false), ((IEnumerable<ulong>)model.Roles.GetValueOrDefault(Array.Empty<ulong>())).ToImmutableArray(), model.User.IsSpecified ? new ulong?(model.User.Value.Id) : ((ulong?)null), model.IsAvailable.ToNullable());
	}
	...
	{
		return new RoleTags(model.BotId.ToNullable(), model.IntegrationId.ToNullable(), model.IsPremiumSubscriber.IsSpecified, model.SubscriptionListingId.ToNullable(), model.IsAvailableForPurchase.IsSpecified, model.GuildConnections.IsSpecified);
	}
	...
	public static Message ToMessage(this InteractionResponse model, IDiscordInteraction interaction)
	{
		if (model.Data.IsSpecified)
		{
			...
			Optional<IMessageComponent[]> components = value.Components;
			IMessageComponent[] defaultValue = Array.Empty<Discord.API.ActionRowComponent>();
			obj.Components = components.GetValueOrDefault(defaultValue);
			...
			if (interaction is IApplicationCommandInteraction applicationCommandInteraction)
			{
				message.Interaction = new MessageInteraction
				{
					...
					User = new User
					{
						...
						Discriminator = ((applicationCommandInteraction.User.Discriminator == "0000") ? Optional<string>.Unspecified : ((Optional<string>)applicationCommandInteraction.User.Discriminator)),
						...
						GlobalName = applicationCommandInteraction.User.GlobalName
					}
				};
			}
			return message;
	...
	public static GuildScheduledEventRecurrenceRule ToEntity(this Discord.API.GuildScheduledEventRecurrenceRule rule)
	{
		return new GuildScheduledEventRecurrenceRule(rule.StartAt, rule.EndAt, rule.Frequency, rule.Interval, (IReadOnlyCollection<RecurrenceRuleWeekday>)(rule.ByWeekday?.ToReadOnlyCollection() ?? ((object)ImmutableArray<RecurrenceRuleWeekday>.Empty)), rule.ByNWeekday?.Select((GuildScheduledEventRecurrenceRuleByNWeekday x) => new RecurrenceRuleByNWeekday(x.WeekNumber, x.Day)).ToImmutableArray() ?? ImmutableArray<RecurrenceRuleByNWeekday>.Empty, (IReadOnlyCollection<RecurrenceRuleMonth>)(rule.ByMonth?.ToReadOnlyCollection() ?? ((object)ImmutableArray<RecurrenceRuleMonth>.Empty)), (IReadOnlyCollection<int>)(rule.ByMonthDay?.ToReadOnlyCollection() ?? ((object)ImmutableArray<int>.Empty)), (IReadOnlyCollection<int>)(rule.ByYearDay?.ToReadOnlyCollection() ?? ((object)ImmutableArray<int>.Empty)), rule.Count);
	}

	public static Discord.API.GuildScheduledEventRecurrenceRule ToModel(this GuildScheduledEventRecurrenceRuleProperties rule)
	{
		return new Discord.API.GuildScheduledEventRecurrenceRule
		{
			Frequency = rule.Frequency,
			ByMonthDay = rule.ByMonthDay?.ToArray(),
			ByNWeekday = rule.ByNWeekday?.Select((RecurrenceRuleByNWeekday x) => new GuildScheduledEventRecurrenceRuleByNWeekday
			{
				Day = x.Day,
				WeekNumber = x.Week
			}).ToArray(),
