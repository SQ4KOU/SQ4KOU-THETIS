...
public class RestGuildUser : RestUser, IGuildUser, IUser, ISnowflakeEntity, IEntity<ulong>, IMentionable, IPresence, IVoiceState
{
	...
	public string DisplayName => Nickname ?? base.GlobalName ?? base.Username;
		...
		: base(discord, id)
	{
		...
		GuildId = guildId ?? (Guild ?? throw new ArgumentNullException("guild", "Expected either guild or guildId to be non-null")).Id;
	}
	...
	{
		return CDN.GetGuildUserBannerUrl(base.Id, GuildId, GuildBannerHash, size, format);
	}
