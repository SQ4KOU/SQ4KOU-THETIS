...
public abstract class RestApplicationCommand : RestEntity<ulong>, IApplicationCommand, ISnowflakeEntity, IEntity<ulong>, IDeletable
{
	...
	internal virtual void Update(ApplicationCommand model)
	{
		...
		Options = (model.Options.IsSpecified ? model.Options.Value.Select(RestApplicationCommandOption.Create).ToImmutableArray() : ImmutableArray.Create<RestApplicationCommandOption>());
