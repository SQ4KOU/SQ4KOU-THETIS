...
public class DiscordRestClient : BaseDiscordClient, IDiscordClient, IDisposable, IAsyncDisposable, IRestClientProvider
{
	...
	{
		User model = await base.ApiClient.GetMyUserAsync(options);
	...
	{
		return RestGuildUser.Create(this, null, await base.ApiClient.GetCurrentUserGuildMember(guildId, options), guildId);
	}
	...
	{
		Application model = await ClientHelper.ModifyCurrentBotApplicationAsync(this, args, options);
		if (_currentBotApplication == null)
		{
	...
	{
		return ClientHelper.GetInviteAsync(this, inviteId, options, scheduledEventId);
	}
	...
	{
		return ClientHelper.AddRoleAsync(this, guildId, userId, roleId, options);
	}

	public Task RemoveRoleAsync(ulong guildId, ulong userId, ulong roleId, RequestOptions options = null)
	{
		return ClientHelper.RemoveRoleAsync(this, guildId, userId, roleId, options);
	}
	...
	{
		return ClientHelper.GetRoleConnectionMetadataRecordsAsync(this, options);
	}
	...
	{
		Preconditions.AtMost(metadata.Count, 5, "metadata", "An application can have a maximum of 5 metadata records.");
	...
	{
		return ClientHelper.GetUserRoleConnectionAsync(applicationId, this, options);
	}
	...
	{
		return ClientHelper.ModifyUserRoleConnectionAsync(applicationId, roleConnection, this, options);
	}
	...
	{
		return base.ApiClient.DeleteEntitlementAsync(entitlementId, options);
	}
	...
	{
		return await CreateGlobalCommand(properties, options).ConfigureAwait(continueOnCapturedContext: false);
	}
	...
	{
		return await BulkOverwriteGlobalCommands(properties, options).ConfigureAwait(continueOnCapturedContext: false);
	}
}
