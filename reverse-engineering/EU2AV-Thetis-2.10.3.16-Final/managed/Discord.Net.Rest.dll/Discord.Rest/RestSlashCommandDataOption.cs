...
public class RestSlashCommandDataOption : IApplicationCommandInteractionDataOption
{
	...
	internal RestSlashCommandDataOption(RestSlashCommandData data, ApplicationCommandInteractionDataOption model)
	{
		...
		Options = (model.Options.IsSpecified ? model.Options.Value.Select((ApplicationCommandInteractionDataOption x) => new RestSlashCommandDataOption(data, x)).ToImmutableArray() : ImmutableArray.Create<RestSlashCommandDataOption>());
	}
