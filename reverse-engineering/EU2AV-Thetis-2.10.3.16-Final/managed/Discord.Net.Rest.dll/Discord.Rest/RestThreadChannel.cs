...
public class RestThreadChannel : RestTextChannel, IThreadChannel, ITextChannel, IMessageChannel, IChannel, ISnowflakeEntity, IEntity<ulong>, IMentionable, INestedChannel, IGuildChannel, IDeletable, IIntegrationChannel
{
	...
	{
		RestThreadChannel restThreadChannel = new RestThreadChannel(discord, guild, model.Id, guild?.Id ?? model.GuildId.Value, model.ThreadMetadata.GetValueOrDefault()?.CreatedAt.GetValueOrDefault());
	...
	internal override void Update(Channel model)
	{
		...
		OwnerId = model.OwnerId.GetValueOrDefault(0uL);
		...
		AppliedTags = ((IEnumerable<ulong>)model.AppliedTags.GetValueOrDefault(Array.Empty<ulong>())).ToImmutableArray();
	}
	...
	{
		return ThreadHelper.GetUsersAsync(this, base.Discord, limit, null, options);
	}
	...
	{
		return (await GetThreadUsersAsync(100, options).FlattenAsync()).ToImmutableArray();
	}
