...
public class SKBitmap : SKObject, ISKSkipObjectRegistration
{
	...
	{
		IntPtr length;
		return new Span<byte>((void*)GetPixels(out length), (int)length);
	}

	public Span<byte> GetPixelSpan(int x, int y)
	{
		return GetPixelSpan().Slice(Info.GetPixelBytesOffset(x, y));
	}
	...
	{
		return DecodeBounds(MemoryExtensions.AsSpan(buffer));
	}
	...
	public static SKBitmap Decode(SKCodec codec, SKImageInfo bitmapInfo)
	{
		...
		SKCodecResult pixels = codec.GetPixels(bitmapInfo, sKBitmap.GetPixels(out var _));
	...
	{
		return Decode(MemoryExtensions.AsSpan(buffer));
	}

	public static SKBitmap Decode(byte[] buffer, SKImageInfo bitmapInfo)
	{
		return Decode(MemoryExtensions.AsSpan(buffer), bitmapInfo);
	}
	...
	{
		SKImageInfoNative sKImageInfoNative = SKImageInfoNative.FromManaged(ref info);
		SKBitmapReleaseDelegate sKBitmapReleaseDelegate = ((releaseProc != null && context != null) ? ((SKBitmapReleaseDelegate)delegate(IntPtr addr, object _)
		{
			releaseProc(addr, context);
		}) : releaseProc);
		DelegateProxies.Create(sKBitmapReleaseDelegate, out var _, out var contextPtr);
		SKBitmapReleaseProxyDelegate releaseProc2 = ((sKBitmapReleaseDelegate != null) ? DelegateProxies.SKBitmapReleaseProxy : null);
	...
	{
		return Resize(info, quality.ToSamplingOptions());
	}
	...
	{
		return Resize(size, quality.ToSamplingOptions());
	}
	...
	public SKBitmap Resize(SKImageInfo info, SKSamplingOptions sampling)
	{
		...
		if (ScalePixels(sKBitmap, sampling))
		{
			return sKBitmap;
		}
		sKBitmap.Dispose();
	...
	{
		return Resize(Info.WithSize(size), sampling);
	}
	...
	public bool ScalePixels(SKBitmap destination, SKSamplingOptions sampling)
	{
		...
		return ScalePixels(destination2, sampling);
	}
	...
	public bool ScalePixels(SKPixmap destination, SKSamplingOptions sampling)
	{
		...
		using SKPixmap sKPixmap = PeekPixels();
		return sKPixmap.ScalePixels(destination, sampling);
	}
	...
	{
		return ToShader(tmx, tmy, SKSamplingOptions.Default, null);
	}

	public unsafe SKShader ToShader(SKShaderTileMode tmx, SKShaderTileMode tmy, SKSamplingOptions sampling)
	{
		return ToShader(tmx, tmy, sampling, null);
	}

	[Obsolete("Use ToShader(SKShaderTileMode tmx, SKShaderTileMode tmy, SKSamplingOptions sampling) instead.")]
	public unsafe SKShader ToShader(SKShaderTileMode tmx, SKShaderTileMode tmy, SKFilterQuality quality)
	{
		return ToShader(tmx, tmy, quality.ToSamplingOptions(), null);
	}

	public unsafe SKShader ToShader(SKShaderTileMode tmx, SKShaderTileMode tmy, SKMatrix localMatrix)
	{
		return ToShader(tmx, tmy, SKSamplingOptions.Default, &localMatrix);
	}

	public unsafe SKShader ToShader(SKShaderTileMode tmx, SKShaderTileMode tmy, SKSamplingOptions sampling, SKMatrix localMatrix)
	{
		return ToShader(tmx, tmy, sampling, &localMatrix);
	}

	[Obsolete("Use ToShader(SKShaderTileMode tmx, SKShaderTileMode tmy, SKSamplingOptions sampling, SKMatrix localMatrix) instead.")]
	public unsafe SKShader ToShader(SKShaderTileMode tmx, SKShaderTileMode tmy, SKFilterQuality quality, SKMatrix localMatrix)
	{
		return ToShader(tmx, tmy, quality.ToSamplingOptions(), &localMatrix);
	}

	private unsafe SKShader ToShader(SKShaderTileMode tmx, SKShaderTileMode tmy, SKSamplingOptions sampling, SKMatrix* localMatrix)
	{
		return SKShader.GetObject(SkiaApi.sk_bitmap_make_shader(Handle, tmx, tmy, &sampling, localMatrix));
	}
}
