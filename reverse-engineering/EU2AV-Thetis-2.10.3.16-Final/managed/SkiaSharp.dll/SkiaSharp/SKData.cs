...
public class SKData : SKObject, ISKNonVirtualReferenceCounted, ISKReferenceCounted
{
	...
	{
		SKDataReleaseDelegate sKDataReleaseDelegate = ((releaseProc != null && context != null) ? ((SKDataReleaseDelegate)delegate(IntPtr addr, object _)
		{
			releaseProc(addr, context);
		}) : releaseProc);
		DelegateProxies.Create(sKDataReleaseDelegate, out var _, out var contextPtr);
		SKDataReleaseProxyDelegate proc = ((sKDataReleaseDelegate != null) ? DelegateProxies.SKDataReleaseProxy : null);
