...
public class SKFontManager : SKObject, ISKReferenceCounted
{
	...
	public unsafe SKTypeface MatchFamily(string familyName, SKFontStyle style)
	{
		...
		fixed (byte* encodedText = StringUtilities.GetEncodedText(familyName, SKTextEncoding.Utf8, addNull: true))
		{
			SKTypeface sKTypeface = SKTypeface.GetObject(SkiaApi.sk_fontmgr_match_family_style(Handle, new IntPtr(encodedText), style.Handle));
	...
	public SKTypeface CreateTypeface(SKStreamAsset stream, int index = 0)
	{
		...
		SKTypeface sKTypeface = SKTypeface.GetObject(SkiaApi.sk_fontmgr_create_from_stream(Handle, stream.Handle, index));
	...
	public unsafe SKTypeface MatchCharacter(string familyName, SKFontStyle style, string[] bcp47, int character)
	{
		...
		fixed (byte* encodedText = StringUtilities.GetEncodedText(familyName, SKTextEncoding.Utf8, addNull: true))
		{
			SKTypeface sKTypeface = SKTypeface.GetObject(SkiaApi.sk_fontmgr_match_family_style_character(Handle, new IntPtr(encodedText), style.Handle, bcp47, (bcp47 != null) ? bcp47.Length : 0, character));
			sKTypeface?.PreventPublicDisposal();
			return sKTypeface;
		}
	}
