...
public class SKDocument : SKObject, ISKReferenceCounted, ISKSkipObjectRegistration
{
	...
	public unsafe static SKDocument CreatePdf(SKWStream stream, SKDocumentPdfMetadata metadata)
	{
		...
		SKDocumentPdfMetadataInternal sKDocumentPdfMetadataInternal = new SKDocumentPdfMetadataInternal
		{
			fTitle = (sKString?.Handle ?? IntPtr.Zero),
			fAuthor = (sKString2?.Handle ?? IntPtr.Zero),
			fSubject = (sKString3?.Handle ?? IntPtr.Zero),
			fKeywords = (sKString4?.Handle ?? IntPtr.Zero),
			fCreator = (sKString5?.Handle ?? IntPtr.Zero),
			fProducer = (sKString6?.Handle ?? IntPtr.Zero),
		...
		if (metadata.Creation.HasValue)
		{
			...
			sKDocumentPdfMetadataInternal.fCreation = &sKTimeDateTimeInternal;
		}
		if (metadata.Modified.HasValue)
		{
			...
			sKDocumentPdfMetadataInternal.fModified = &sKTimeDateTimeInternal2;
		}
		return SKObject.Referenced(GetObject(SkiaApi.sk_document_create_pdf_from_stream_with_metadata(stream.Handle, &sKDocumentPdfMetadataInternal)), stream);
	}
