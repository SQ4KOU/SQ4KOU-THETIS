...
public struct SKRect(float left, float top, float right, float bottom) : IEquatable<SKRect>
{
	public static readonly SKRect Empty;

	private float left = left;

	private float top = top;

	private float right = right;

	private float bottom = bottom;
