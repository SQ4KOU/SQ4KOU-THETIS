...
public class SKTextBlob : SKObject, ISKNonVirtualReferenceCounted, ISKReferenceCounted, ISKSkipObjectRegistration
{
	...
	{
		return Create(MemoryExtensions.AsSpan(text), font, origin);
	}
	...
	internal unsafe static SKTextBlob? Create(void* text, int length, SKTextEncoding encoding, SKFont font, SKPoint origin)
	{
		...
		SKRawRunBuffer<SKPoint> sKRawRunBuffer = sKTextBlobBuilder.AllocateRawPositionedRun(font, num);
		font.GetGlyphs(text, length, encoding, sKRawRunBuffer.Glyphs);
	...
	{
		return CreateHorizontal(MemoryExtensions.AsSpan(text), font, positions, y);
	}
	...
	internal unsafe static SKTextBlob? CreateHorizontal(void* text, int length, SKTextEncoding encoding, SKFont font, ReadOnlySpan<float> positions, float y)
	{
		...
		using SKTextBlobBuilder sKTextBlobBuilder = new SKTextBlobBuilder();
		SKRawRunBuffer<float> sKRawRunBuffer = sKTextBlobBuilder.AllocateRawHorizontalRun(font, num, y);
		font.GetGlyphs(text, length, encoding, sKRawRunBuffer.Glyphs);
	...
	{
		return CreatePositioned(MemoryExtensions.AsSpan(text), font, positions);
	}
	...
	{
		return CreateRotationScale(MemoryExtensions.AsSpan(text), font, positions);
	}
	...
	internal unsafe static SKTextBlob? CreateRotationScale(void* text, int length, SKTextEncoding encoding, SKFont font, ReadOnlySpan<SKRotationScaleMatrix> positions)
	{
		...
		font.GetGlyphs(text, length, encoding, sKRotationScaleRunBuffer.Glyphs);
		positions.CopyTo(sKRotationScaleRunBuffer.Positions);
		return sKTextBlobBuilder.Build();
	}
	...
	{
		return CreatePathPositioned(MemoryExtensions.AsSpan(text), font, path, textAlign, origin);
	}
