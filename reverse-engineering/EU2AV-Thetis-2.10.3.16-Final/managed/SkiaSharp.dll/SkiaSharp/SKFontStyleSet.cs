...
public class SKFontStyleSet : SKObject, ISKReferenceCounted, IEnumerable<SKFontStyle>, IEnumerable, IReadOnlyCollection<SKFontStyle>, IReadOnlyList<SKFontStyle>
{
	...
	public SKTypeface CreateTypeface(int index)
	{
		...
		SKTypeface sKTypeface = SKTypeface.GetObject(SkiaApi.sk_fontstyleset_create_typeface(Handle, index));
	...
	public SKTypeface CreateTypeface(SKFontStyle style)
	{
		...
		SKTypeface sKTypeface = SKTypeface.GetObject(SkiaApi.sk_fontstyleset_match_style(Handle, style.Handle));
		sKTypeface?.PreventPublicDisposal();
		GC.KeepAlive(this);
		return sKTypeface;
	}
