...
public class SKCodec : SKObject, ISKSkipObjectRegistration
{
	...
	public byte[] Pixels
	{
		get
		{
			SKCodecResult pixels = GetPixels(out var pixels2);
			if (pixels != SKCodecResult.Success && pixels != SKCodecResult.IncompleteInput)
			{
				throw new Exception(pixels.ToString());
			}
			return pixels2;
		}
	}
	...
	public unsafe SKCodecResult GetPixels(SKImageInfo info, IntPtr pixels, int rowBytes, SKCodecOptions options)
	{
		...
		return SkiaApi.sk_codec_get_pixels(Handle, &sKImageInfoNative, (void*)pixels, (IntPtr)rowBytes, &sKCodecOptionsInternal);
	}
	...
	public unsafe SKCodecResult StartIncrementalDecode(SKImageInfo info, IntPtr pixels, int rowBytes, SKCodecOptions options)
	{
		...
		return SkiaApi.sk_codec_start_incremental_decode(Handle, &sKImageInfoNative, (void*)pixels, (IntPtr)rowBytes, &sKCodecOptionsInternal);
	}
	...
	public unsafe SKCodecResult StartScanlineDecode(SKImageInfo info, SKCodecOptions options)
	{
		...
		return SkiaApi.sk_codec_start_scanline_decode(Handle, &sKImageInfoNative, &sKCodecOptionsInternal);
	}
	...
	public unsafe static SKCodec Create(SKStream stream, out SKCodecResult result)
	{
		...
		{
			SKCodec sKCodec = GetObject(SkiaApi.sk_codec_new_from_stream(stream.Handle, result2));
			stream.RevokeOwnership(sKCodec);
			return sKCodec;
		}
	}
