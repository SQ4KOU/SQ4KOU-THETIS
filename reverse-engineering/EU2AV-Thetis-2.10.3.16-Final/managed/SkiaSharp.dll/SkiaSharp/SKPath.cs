...
public class SKPath : SKObject, ISKSkipObjectRegistration
{
	...
	{
		this.Transform(in ILSpyHelper_AsRefReadOnly(SKMatrix.CreateTranslation(dx, dy)));
	...
	public unsafe void Transform(in SKMatrix matrix)
	{
		fixed (SKMatrix* cmatrix = &matrix)
		{
			SkiaApi.sk_path_transform(Handle, cmatrix);
		}
	}

	public unsafe void Transform(in SKMatrix matrix, SKPath destination)
	{
		if (destination == null)
		{
		...
		fixed (SKMatrix* cmatrix = &matrix)
		{
			SkiaApi.sk_path_transform_to_dest(Handle, cmatrix, destination.Handle);
		}
	}
