...
public class SKPicture : SKObject, ISKReferenceCounted
{
	...
	public int ApproximateBytesUsed => (int)SkiaApi.sk_picture_approximate_bytes_used(Handle);
	...
	{
		return SkiaApi.sk_picture_approximate_op_count(Handle, includeNested);
	}
	...
	{
		return ToShader(SKShaderTileMode.Clamp, SKShaderTileMode.Clamp, SKFilterMode.Nearest, null, null);
	}

	public unsafe SKShader ToShader(SKShaderTileMode tmx, SKShaderTileMode tmy)
	{
		return ToShader(tmx, tmy, SKFilterMode.Nearest, null, null);
	}

	public unsafe SKShader ToShader(SKShaderTileMode tmx, SKShaderTileMode tmy, SKFilterMode filterMode)
	{
		return ToShader(tmx, tmy, filterMode, null, null);
	}

	public unsafe SKShader ToShader(SKShaderTileMode tmx, SKShaderTileMode tmy, SKRect tile)
	{
		return ToShader(tmx, tmy, SKFilterMode.Nearest, null, &tile);
	}

	public unsafe SKShader ToShader(SKShaderTileMode tmx, SKShaderTileMode tmy, SKFilterMode filterMode, SKRect tile)
	{
		return ToShader(tmx, tmy, filterMode, null, &tile);
	}

	public unsafe SKShader ToShader(SKShaderTileMode tmx, SKShaderTileMode tmy, SKMatrix localMatrix, SKRect tile)
	{
		return ToShader(tmx, tmy, SKFilterMode.Nearest, &localMatrix, &tile);
	}

	public unsafe SKShader ToShader(SKShaderTileMode tmx, SKShaderTileMode tmy, SKFilterMode filterMode, SKMatrix localMatrix, SKRect tile)
	{
		return ToShader(tmx, tmy, filterMode, &localMatrix, &tile);
	}

	private unsafe SKShader ToShader(SKShaderTileMode tmx, SKShaderTileMode tmy, SKFilterMode filterMode, SKMatrix* localMatrix, SKRect* tile)
	{
		return SKShader.GetObject(SkiaApi.sk_picture_make_shader(Handle, tmx, tmy, filterMode, localMatrix, tile));
	}
