...
internal sealed class SynthesizedReadOnlyListConstructor : SynthesizedInstanceConstructor
{
	...
	internal SynthesizedReadOnlyListConstructor(SynthesizedReadOnlyListTypeSymbol containingType, TypeSymbol parameterType, string parameterName)
		: base(containingType)
	{
		Parameters = ImmutableArray.Create(SynthesizedParameterSymbol.Create(this, TypeWithAnnotations.Create(parameterType), 0, RefKind.None, parameterName));
	}
	...
	internal override void GenerateMethodBody(TypeCompilationState compilationState, BindingDiagnosticBag diagnostics)
	{
		...
		try
		{
			...
			FieldSymbol f = ContainingType.GetFieldsToEmit().Single();
