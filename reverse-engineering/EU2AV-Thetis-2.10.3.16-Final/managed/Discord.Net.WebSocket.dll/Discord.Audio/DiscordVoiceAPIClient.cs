...
internal class DiscordVoiceAPIClient : IDisposable
{
	...
	internal DiscordVoiceAPIClient(ulong guildId, WebSocketProvider webSocketProvider, UdpSocketProvider udpSocketProvider, JsonSerializer serializer = null)
	{
		...
		_udp.ReceivedDatagram += delegate(byte[] data, int index, int count)
		{
		...
		WebSocketClient.TextMessage += delegate(string text)
		{
			SocketFrame socketFrame = JsonConvert.DeserializeObject<SocketFrame>(text);
			return _receivedEvent.InvokeAsync((VoiceOpCode)socketFrame.Operation, socketFrame.Payload);
		};
	...
	{
		return SendAsync(VoiceOpCode.Heartbeat, DateTimeOffset.UtcNow.ToUnixTimeMilliseconds(), options);
	}
	...
	public async Task SendDiscoveryAsync(uint ssrc)
	{
		...
		array[1] = 1;
		array[3] = 70;
		array[4] = (byte)(ssrc >> 24);
		array[5] = (byte)(ssrc >> 16);
		array[6] = (byte)(ssrc >> 8);
		array[7] = (byte)ssrc;
		await SendAsync(array, 0, 74).ConfigureAwait(continueOnCapturedContext: false);
