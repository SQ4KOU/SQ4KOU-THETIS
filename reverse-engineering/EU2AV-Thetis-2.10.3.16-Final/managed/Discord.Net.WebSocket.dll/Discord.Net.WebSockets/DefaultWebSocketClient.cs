...
internal class DefaultWebSocketClient : IWebSocketClient, IDisposable
{
	...
	private async Task DisconnectInternalAsync(int closeCode = 1000, bool isDisposing = false)
	{
		...
		try
		{
			await (_task ?? Task.CompletedTask).ConfigureAwait(continueOnCapturedContext: false);
			_task = null;
		}
		finally
		{
		...
		try
		{
			while (!cancelToken.IsCancellationRequested)
			{
				...
				if (webSocketReceiveResult.MessageType == WebSocketMessageType.Text)
				{
					...
					await TextMessage(arg).ConfigureAwait(continueOnCapturedContext: false);
				}
				else
				{
					await BinaryMessage(array, 0, num).ConfigureAwait(continueOnCapturedContext: false);
				}
			}
		}
		catch (Win32Exception ex) when (ex.HResult == -2147012894)
		{
			OnClosed(new WebSocketException(WebSocketError.ConnectionClosedPrematurely, "Connection timed out.", ex));
		}
