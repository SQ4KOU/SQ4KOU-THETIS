...
public class SocketThreadChannel : SocketTextChannel, IThreadChannel, ITextChannel, IMessageChannel, IChannel, ISnowflakeEntity, IEntity<ulong>, IMentionable, INestedChannel, IGuildChannel, IDeletable, IIntegrationChannel
{
	...
	internal SocketThreadUser AddOrUpdateThreadMember(ThreadMember model, SocketGuildUser guildMember = null)
	{
		...
		{
			value = SocketThreadUser.Create(base.Guild, this, model, guildMember ?? base.Guild.GetUser(model.UserId.GetValueOrDefault(0uL)));
	...
	public async Task DownloadUsersAsync(RequestOptions options = null)
	{
		...
		while (num == 100)
		{
			ThreadMember[] array = await base.Discord.ApiClient.ListThreadMembersAsync(base.Id, after, 100, options);
			num = array.Length;
			after = array.Max((ThreadMember x) => x.UserId.GetValueOrDefault());
			lock (_downloadLock)
			{
				ThreadMember[] array2 = array;
				foreach (ThreadMember model in array2)
				{
					AddOrUpdateThreadMember(model);
				}
			}
		}
	}
