...
internal class MessageCache
{
	...
	public MessageCache(DiscordSocketClient discord)
	{
		...
		int size = _size;
		_messages = new ConcurrentDictionary<ulong, SocketMessage>(ConcurrentHashSet.DefaultConcurrencyLevel, (!((double)size * 1.05 > 2147483647.0)) ? ((int)((double)size * 1.05)) : int.MaxValue);
