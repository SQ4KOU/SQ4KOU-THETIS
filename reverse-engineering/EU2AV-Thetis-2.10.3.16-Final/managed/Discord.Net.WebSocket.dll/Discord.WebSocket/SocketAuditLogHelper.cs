...
{
	private static readonly Dictionary<ActionType, Func<DiscordSocketClient, AuditLogEntry, ISocketAuditLogData>> CreateMapping = new Dictionary<ActionType, Func<DiscordSocketClient, AuditLogEntry, ISocketAuditLogData>>
	{
	...
	public static ISocketAuditLogData CreateData(DiscordSocketClient discord, AuditLogEntry entry)
	{
		if (CreateMapping.TryGetValue(entry.Action, out var value))
		{
			return value(discord, entry);
		}
		return null;
	}
}
