...
public class SocketApplicationCommand : SocketEntity<ulong>, IApplicationCommand, ISnowflakeEntity, IEntity<ulong>, IDeletable
{
	...
	internal void Update(ApplicationCommand model)
	{
		...
		Type = model.Type;
		Options = (model.Options.IsSpecified ? model.Options.Value.Select(SocketApplicationCommandOption.Create).ToImmutableArray() : ImmutableArray.Create<SocketApplicationCommandOption>());
		...
		IsNsfw = model.Nsfw.GetValueOrDefault(false) ?? false;
