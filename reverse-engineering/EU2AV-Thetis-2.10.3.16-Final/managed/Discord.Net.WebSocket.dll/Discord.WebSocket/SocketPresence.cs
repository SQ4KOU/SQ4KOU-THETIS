...
public class SocketPresence : IPresence
{
	...
	{
		Status = model.Status;
		ActiveClients = (IReadOnlyCollection<ClientType>)(ConvertClientTypesDict(model.ClientStatus.GetValueOrDefault()) ?? ((object)ImmutableArray<ClientType>.Empty));
		Activities = (IReadOnlyCollection<IActivity>)(ConvertActivitiesList(model.Activities) ?? ((object)ImmutableArray<IActivity>.Empty));
	}
