...
public class SocketApplicationCommandOption : IApplicationCommandOption
{
	...
	internal void Update(ApplicationCommandOption model)
	{
		...
		Choices = (model.Choices.IsSpecified ? model.Choices.Value.Select(SocketApplicationCommandChoice.Create).ToImmutableArray() : ImmutableArray.Create<SocketApplicationCommandChoice>());
		Options = (model.Options.IsSpecified ? model.Options.Value.Select(Create).ToImmutableArray() : ImmutableArray.Create<SocketApplicationCommandOption>());
