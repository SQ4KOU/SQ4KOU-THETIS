...
public class DiscordShardedClient : BaseSocketClient, IDiscordClient, IDisposable, IAsyncDisposable
{
	...
	internal Task AcquireIdentifyLockAsync(int shardId, CancellationToken token)
	{
		...
		return _identifySemaphores[num].WaitAsync(token);
	}
	...
	{
		return Task.WhenAll(_shards.Select((DiscordSocketClient x) => x.StartAsync()));
	}

	public override Task StopAsync()
	{
		return Task.WhenAll(_shards.Select((DiscordSocketClient x) => x.StopAsync()));
	}
	...
	{
		return _shards[0].GetApplicationInfoAsync(options);
	}
	...
	{
		CustomStatusGame statusGame = new CustomStatusGame(status);
		for (int i = 0; i < _shards.Length; i++)
		{
			await _shards[i].SetActivityAsync(statusGame).ConfigureAwait(continueOnCapturedContext: false);
		}
	}
	...
	private void RegisterEvents(DiscordSocketClient client, bool isPrimary)
	{
		...
		client.SentRequest += (string method, string endpoint, double millis) => _sentRequest.InvokeAsync(method, endpoint, millis);
		...
		client.AutoModActionExecuted += (SocketGuild guild, AutoModRuleAction action, AutoModActionExecutedData arg3) => _autoModActionExecuted.InvokeAsync(guild, action, arg3);
		client.AutoModRuleCreated += (SocketAutoModRule rule) => _autoModRuleCreated.InvokeAsync(rule);
		client.AutoModRuleDeleted += (SocketAutoModRule rule) => _autoModRuleDeleted.InvokeAsync(rule);
		client.AutoModRuleUpdated += (Cacheable<SocketAutoModRule, ulong> arg1, SocketAutoModRule arg2) => _autoModRuleUpdated.InvokeAsync(arg1, arg2);
		client.WebhooksUpdated += (SocketGuild arg1, SocketChannel arg2) => _webhooksUpdated.InvokeAsync(arg1, arg2);
		client.AuditLogCreated += (SocketAuditLogEntry arg1, SocketGuild arg2) => _auditLogCreated.InvokeAsync(arg1, arg2);
		client.VoiceChannelStatusUpdated += (Cacheable<SocketVoiceChannel, ulong> arg1, string arg2, string arg3) => _voiceChannelStatusUpdated.InvokeAsync(arg1, arg2, arg3);
		client.EntitlementCreated += (SocketEntitlement arg1) => _entitlementCreated.InvokeAsync(arg1);
		client.EntitlementUpdated += (Cacheable<SocketEntitlement, ulong> arg1, SocketEntitlement arg2) => _entitlementUpdated.InvokeAsync(arg1, arg2);
		client.EntitlementDeleted += (Cacheable<SocketEntitlement, ulong> arg1) => _entitlementDeleted.InvokeAsync(arg1);
		client.PollVoteAdded += (Cacheable<IUser, ulong> arg1, Cacheable<ISocketMessageChannel, IRestMessageChannel, IMessageChannel, ulong> arg2, Cacheable<IUserMessage, ulong> arg3, Cacheable<SocketGuild, RestGuild, IGuild, ulong>? arg4, ulong arg5) => _pollVoteAdded.InvokeAsync(arg1, arg2, arg3, arg4, arg5);
		client.PollVoteRemoved += (Cacheable<IUser, ulong> arg1, Cacheable<ISocketMessageChannel, IRestMessageChannel, IMessageChannel, ulong> arg2, Cacheable<IUserMessage, ulong> arg3, Cacheable<SocketGuild, RestGuild, IGuild, ulong>? arg4, ulong arg5) => _pollVoteRemoved.InvokeAsync(arg1, arg2, arg3, arg4, arg5);
	}
	...
	public async Task<SocketApplicationCommand> CreateGlobalApplicationCommandAsync(ApplicationCommandProperties properties, RequestOptions options = null)
	{
		...
		SocketApplicationCommand socketApplicationCommand = null;
		DiscordSocketClient[] shards = _shards;
		foreach (DiscordSocketClient shard in shards)
		{
			socketApplicationCommand = shard.State.GetOrAddCommand(model.Id, (ulong id) => SocketApplicationCommand.Create(shard, model));
	...
	{
		ApplicationCommand[] source = await InteractionHelper.BulkOverwriteGlobalCommandsAsync(this, properties, options);
		IEnumerable<SocketApplicationCommand> enumerable = null;
		DiscordSocketClient[] shards = _shards;
		foreach (DiscordSocketClient shard in shards)
		{
			enumerable = source.Select((ApplicationCommand x) => SocketApplicationCommand.Create(shard, x));
			shard.State.PurgeCommands((SocketApplicationCommand x) => x.IsGlobalCommand);
			foreach (SocketApplicationCommand item in enumerable)
			{
				shard.State.AddCommand(item);
			}
		}
