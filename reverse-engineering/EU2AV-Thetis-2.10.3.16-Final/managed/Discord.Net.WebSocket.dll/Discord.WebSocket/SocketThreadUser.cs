...
public class SocketThreadUser : SocketUser, IThreadUser, IMentionable, IGuildUser, IUser, ISnowflakeEntity, IEntity<ulong>, IPresence, IVoiceState
{
	...
	public string DisplayName => GuildUser.Nickname ?? GuildUser.GlobalName ?? GuildUser.Username;
	...
	public string GuildBannerHash => GuildUser.GuildBannerHash;
	...
	public GuildUserFlags Flags => GuildUser.Flags;
	...
	internal static SocketThreadUser Create(SocketGuild guild, SocketThreadChannel thread, ThreadMember model, SocketGuildUser guildUser = null)
	{
		SocketThreadUser socketThreadUser = new SocketThreadUser(guild, thread, model.UserId.Value, guildUser);
	...
	{
		SocketThreadUser socketThreadUser = new SocketThreadUser(guild, thread, owner.Id, owner);
	...
	{
		return GuildUser.GetGuildBannerUrl(format, size);
	}

	public override string GetDisplayAvatarUrl(ImageFormat format = ImageFormat.Auto, ushort size = 128)
	{
		return GuildUser.GetGuildAvatarUrl(ImageFormat.Auto, 128) ?? base.GetDisplayAvatarUrl(format, size);
	}
