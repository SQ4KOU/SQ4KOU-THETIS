...
public class SocketGuildEvent : SocketEntity<ulong>, IGuildScheduledEvent, IEntity<ulong>
{
	...
	internal void Update(GuildScheduledEvent model)
	{
		...
		{
			SocketGuildUser user = Guild.GetUser(model.CreatorId.GetValueOrDefault(0uL).GetValueOrDefault());
			if (user != null)
			{
	...
	{
		return CDN.GetEventCoverImageUrl(GuildId, base.Id, CoverImageId, format, size);
	}
