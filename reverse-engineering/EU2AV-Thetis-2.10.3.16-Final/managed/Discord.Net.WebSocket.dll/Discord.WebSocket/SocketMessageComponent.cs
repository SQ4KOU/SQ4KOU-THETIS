...
public class SocketMessageComponent : SocketInteraction, IComponentInteraction, IDiscordInteraction, ISnowflakeEntity, IEntity<ulong>
{
	...
	{
		Data = new SocketMessageComponentData(model.Data.IsSpecified ? ((MessageComponentInteractionData)model.Data.Value) : null, client, client.State, client.Guilds.FirstOrDefault((SocketGuild x) => x.Id == model.GuildId.GetValueOrDefault()), model.User.GetValueOrDefault());
	}
