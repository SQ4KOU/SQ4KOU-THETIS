...
public abstract class BaseSocketClient : BaseDiscordClient, IDiscordClient, IDisposable, IAsyncDisposable, IRestClientProvider
{
	...
	internal readonly AsyncEvent<Func<Cacheable<SocketVoiceChannel, ulong>, string, string, Task>> _voiceChannelStatusUpdated = new AsyncEvent<Func<Cacheable<SocketVoiceChannel, ulong>, string, string, Task>>();
	...
	internal readonly AsyncEvent<Func<Cacheable<IUser, ulong>, Cacheable<ISocketMessageChannel, IRestMessageChannel, IMessageChannel, ulong>, Cacheable<IUserMessage, ulong>, Cacheable<SocketGuild, RestGuild, IGuild, ulong>?, ulong, Task>> _pollVoteAdded = new AsyncEvent<Func<Cacheable<IUser, ulong>, Cacheable<ISocketMessageChannel, IRestMessageChannel, IMessageChannel, ulong>, Cacheable<IUserMessage, ulong>, Cacheable<SocketGuild, RestGuild, IGuild, ulong>?, ulong, Task>>();

	internal readonly AsyncEvent<Func<Cacheable<IUser, ulong>, Cacheable<ISocketMessageChannel, IRestMessageChannel, IMessageChannel, ulong>, Cacheable<IUserMessage, ulong>, Cacheable<SocketGuild, RestGuild, IGuild, ulong>?, ulong, Task>> _pollVoteRemoved = new AsyncEvent<Func<Cacheable<IUser, ulong>, Cacheable<ISocketMessageChannel, IRestMessageChannel, IMessageChannel, ulong>, Cacheable<IUserMessage, ulong>, Cacheable<SocketGuild, RestGuild, IGuild, ulong>?, ulong, Task>>();
	...
	internal readonly AsyncEvent<Func<SocketAuditLogEntry, SocketGuild, Task>> _auditLogCreated = new AsyncEvent<Func<SocketAuditLogEntry, SocketGuild, Task>>();

	internal readonly AsyncEvent<Func<SocketAutoModRule, Task>> _autoModRuleCreated = new AsyncEvent<Func<SocketAutoModRule, Task>>();

	internal readonly AsyncEvent<Func<Cacheable<SocketAutoModRule, ulong>, SocketAutoModRule, Task>> _autoModRuleUpdated = new AsyncEvent<Func<Cacheable<SocketAutoModRule, ulong>, SocketAutoModRule, Task>>();

	internal readonly AsyncEvent<Func<SocketAutoModRule, Task>> _autoModRuleDeleted = new AsyncEvent<Func<SocketAutoModRule, Task>>();

	internal readonly AsyncEvent<Func<SocketGuild, AutoModRuleAction, AutoModActionExecutedData, Task>> _autoModActionExecuted = new AsyncEvent<Func<SocketGuild, AutoModRuleAction, AutoModActionExecutedData, Task>>();

	internal readonly AsyncEvent<Func<SocketEntitlement, Task>> _entitlementCreated = new AsyncEvent<Func<SocketEntitlement, Task>>();

	internal readonly AsyncEvent<Func<Cacheable<SocketEntitlement, ulong>, SocketEntitlement, Task>> _entitlementUpdated = new AsyncEvent<Func<Cacheable<SocketEntitlement, ulong>, SocketEntitlement, Task>>();

	internal readonly AsyncEvent<Func<Cacheable<SocketEntitlement, ulong>, Task>> _entitlementDeleted = new AsyncEvent<Func<Cacheable<SocketEntitlement, ulong>, Task>>();

	internal readonly AsyncEvent<Func<SocketSubscription, Task>> _subscriptionCreated = new AsyncEvent<Func<SocketSubscription, Task>>();

	internal readonly AsyncEvent<Func<Cacheable<SocketSubscription, ulong>, SocketSubscription, Task>> _subscriptionUpdated = new AsyncEvent<Func<Cacheable<SocketSubscription, ulong>, SocketSubscription, Task>>();

	internal readonly AsyncEvent<Func<Cacheable<SocketSubscription, ulong>, Task>> _subscriptionDeleted = new AsyncEvent<Func<Cacheable<SocketSubscription, ulong>, Task>>();

	internal readonly AsyncEvent<Func<string, JToken, Task>> _unknownDispatchReceived = new AsyncEvent<Func<string, JToken, Task>>();
	...
	public event Func<Cacheable<SocketVoiceChannel, ulong>, string, string, Task> VoiceChannelStatusUpdated
	{
		add
		{
			_voiceChannelStatusUpdated.Add(value);
		}
		remove
		{
			_voiceChannelStatusUpdated.Remove(value);
		}
	}
	...
	public event Func<Cacheable<IUser, ulong>, Cacheable<ISocketMessageChannel, IRestMessageChannel, IMessageChannel, ulong>, Cacheable<IUserMessage, ulong>, Cacheable<SocketGuild, RestGuild, IGuild, ulong>?, ulong, Task> PollVoteAdded
	{
		add
		{
			_pollVoteAdded.Add(value);
		}
		remove
		{
			_pollVoteAdded.Remove(value);
		}
	}
	...
	public event Func<Cacheable<IUser, ulong>, Cacheable<ISocketMessageChannel, IRestMessageChannel, IMessageChannel, ulong>, Cacheable<IUserMessage, ulong>, Cacheable<SocketGuild, RestGuild, IGuild, ulong>?, ulong, Task> PollVoteRemoved
	{
		add
		{
			_pollVoteRemoved.Add(value);
		}
		remove
		{
			_pollVoteRemoved.Remove(value);
		}
	}
	...
	public event Func<SocketAuditLogEntry, SocketGuild, Task> AuditLogCreated
	{
		add
		{
			_auditLogCreated.Add(value);
		}
		remove
		{
			_auditLogCreated.Remove(value);
		}
	}
	...
	public event Func<SocketAutoModRule, Task> AutoModRuleCreated
	{
		add
		{
			_autoModRuleCreated.Add(value);
		}
		remove
		{
			_autoModRuleCreated.Remove(value);
		}
	}
	...
	public event Func<Cacheable<SocketAutoModRule, ulong>, SocketAutoModRule, Task> AutoModRuleUpdated
	{
		add
		{
			_autoModRuleUpdated.Add(value);
		}
		remove
		{
			_autoModRuleUpdated.Remove(value);
		}
	}
	...
	public event Func<SocketAutoModRule, Task> AutoModRuleDeleted
	{
		add
		{
			_autoModRuleDeleted.Add(value);
		}
		remove
		{
			_autoModRuleDeleted.Remove(value);
		}
	}
	...
	public event Func<SocketGuild, AutoModRuleAction, AutoModActionExecutedData, Task> AutoModActionExecuted
	{
		add
		{
			_autoModActionExecuted.Add(value);
		}
		remove
		{
			_autoModActionExecuted.Remove(value);
		}
	}
	...
	public event Func<SocketEntitlement, Task> EntitlementCreated
	{
		add
		{
			_entitlementCreated.Add(value);
		}
		remove
		{
			_entitlementCreated.Remove(value);
		}
	}
	...
	public event Func<Cacheable<SocketEntitlement, ulong>, SocketEntitlement, Task> EntitlementUpdated
	{
		add
		{
			_entitlementUpdated.Add(value);
		}
		remove
		{
			_entitlementUpdated.Remove(value);
		}
	}
	...
	public event Func<Cacheable<SocketEntitlement, ulong>, Task> EntitlementDeleted
	{
		add
		{
			_entitlementDeleted.Add(value);
		}
		remove
		{
			_entitlementDeleted.Remove(value);
		}
	}
	...
	public event Func<SocketSubscription, Task> SubscriptionCreated
	{
		add
		{
			_subscriptionCreated.Add(value);
		}
		remove
		{
			_subscriptionCreated.Remove(value);
		}
	}
	...
	public event Func<Cacheable<SocketSubscription, ulong>, SocketSubscription, Task> SubscriptionUpdated
	{
		add
		{
			_subscriptionUpdated.Add(value);
		}
		remove
		{
			_subscriptionUpdated.Remove(value);
		}
	}
	...
	public event Func<Cacheable<SocketSubscription, ulong>, Task> SubscriptionDeleted
	{
		add
		{
			_subscriptionDeleted.Add(value);
		}
		remove
		{
			_subscriptionDeleted.Remove(value);
		}
	}
	...
	public event Func<string, JToken, Task> UnknownDispatchReceived
	{
		add
		{
			_unknownDispatchReceived.Add(value);
		}
		remove
		{
			_unknownDispatchReceived.Remove(value);
		}
	}
	...
	{
		return ClientHelper.GetInviteAsync(this, inviteId, options ?? RequestOptions.Default, scheduledEventId);
	}
