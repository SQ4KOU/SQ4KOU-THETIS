...
internal static class SocketChannelHelper
{
	public static IAsyncEnumerable<IReadOnlyCollection<IMessage>> GetMessagesAsync(ISocketMessageChannel channel, DiscordSocketClient discord, MessageCache messages, ulong? fromMessageId, Direction dir, int limit, CacheMode mode, RequestOptions options)
	{
		...
		IAsyncEnumerable<IReadOnlyCollection<IMessage>> asyncEnumerable = ((IEnumerable<IReadOnlyCollection<IMessage>>)ImmutableArray.Create(cachedMessages)).ToAsyncEnumerable();
		switch (dir)
		{
		...
		case Direction.After:
		{
			...
			{
				fromMessageId = cachedMessages.Max((SocketMessage x) => x.Id);
				limit -= cachedMessages.Count;
				if (limit <= 0)
				{
					return asyncEnumerable;
			...
			IAsyncEnumerable<IReadOnlyCollection<RestMessage>> messagesAsync = ChannelHelper.GetMessagesAsync(channel, discord, fromMessageId, dir, limit, options);
			if (!flag && cachedMessages.Count != 0)
			{
