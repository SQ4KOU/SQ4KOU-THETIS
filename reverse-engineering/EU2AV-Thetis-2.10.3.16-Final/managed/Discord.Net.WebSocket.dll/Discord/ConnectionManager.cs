...
internal class ConnectionManager : IDisposable
{
			...
			try
			{
				...
				while (!reconnectCancelToken.IsCancellationRequested)
				{
					...
					catch (Exception ex)
					{
						...
						{
							await _logger.ErrorAsync(ex).ConfigureAwait(continueOnCapturedContext: false);
							await DisconnectAsync(ex, isReconnecting: false).ConfigureAwait(continueOnCapturedContext: false);
						}
						else
						{
							await _logger.WarningAsync(ex).ConfigureAwait(continueOnCapturedContext: false);
							await DisconnectAsync(ex, isReconnecting: true).ConfigureAwait(continueOnCapturedContext: false);
						}
					}
					if (!reconnectCancelToken.IsCancellationRequested)
					{
	...
	{
		return _readyPromise.TrySetResultAsync(result: true);
	}

	public Task WaitAsync()
	{
		return _readyPromise.Task;
	}
