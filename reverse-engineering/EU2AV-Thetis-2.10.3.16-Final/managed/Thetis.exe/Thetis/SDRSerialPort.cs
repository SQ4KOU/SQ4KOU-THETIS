...
public class SDRSerialPort
{
	...
	private void SerialPinChanged(object source, SerialPinChangedEventArgs e)
	{
		...
		bool flag = e.EventType == SerialPinChange.DsrChanged;
		bool flag2 = e.EventType == SerialPinChange.CtsChanged;
		bool flag3 = flag | flag2;
		if (use_for_keyptt & flag3)
		{
			if (flag)
			{
				if (ptt_on_dtr)
				{
					CWInput.KeyerPTT = dsrHolding;
				}
				if (key_on_dtr)
				{
					NetworkIO.SetCWX(dsrHolding ? 1 : 0);
				}
			}
			else if (flag2)
			{
				if (ptt_on_rts)
				{
					CWInput.KeyerPTT = ctsHolding;
				}
				if (key_on_rts)
				{
					NetworkIO.SetCWX(ctsHolding ? 1 : 0);
				}
			}
		}
		else if (use_for_paddles & flag3)
		{
			if (flag)
			{
				NetworkIO.SetCWDot(dsrHolding ? 1 : 0);
			}
			else if (flag2)
			{
				NetworkIO.SetCWDash(ctsHolding ? 1 : 0);
			}
		}
		if (use_for_cat_ptt)
		{
			if (flag && ptt_on_dtr)
			{
				CWInput.CATPTT = dsrHolding;
			}
			else if (flag2 && ptt_on_rts)
			{
				CWInput.CATPTT = ctsHolding;
			}
		}
	}
