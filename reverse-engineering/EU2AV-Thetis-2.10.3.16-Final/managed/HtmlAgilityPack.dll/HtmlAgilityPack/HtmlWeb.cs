...
public class HtmlWeb
{
	...
	{
		return SharedHttpClient.GetOrAdd(web.UserAgent, (string x) => new HttpClient
		{
			Timeout = TimeSpan.FromMilliseconds(web.Timeout),
			DefaultRequestHeaders = { { "User-Agent", web.UserAgent } }
		});
	}
	...
	public string GetCachePath(Uri uri)
	{
		...
		{
			return Path.Combine(_cachePath, (uri.Host + text.TrimEnd(Path.AltDirectorySeparatorChar)).Replace('/', '\\') + ".htm");
		}
	...
	{
		return IsHtmlContent(GetContentTypeForExtension(Path.GetExtension(path), null));
	}
	...
	public async Task<HtmlDocument> LoadFromWebAsync(Uri uri, Encoding encoding, NetworkCredential credentials, CancellationToken cancellationToken)
	{
		...
		{
			httpClient = GetSharedHttpClient(this);
		}
	...
	public HtmlDocument LoadFromBrowser(string url, Func<object, bool> isBrowserScriptCompleted = null)
	{
		...
		EventInfo eventInfo = disposable.GetType().GetEvent("NewWindow");
		if (eventInfo != null)
		{
			MethodInfo method2 = GetType().GetMethod("Web_NewWindow", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
			Delegate handler = Delegate.CreateDelegate(eventInfo.EventHandlerType, null, method2);
			eventInfo.AddEventHandler(disposable, handler);
		}
	...
	{
		PropertyInfo property = e.GetType().GetProperty("Cancel");
		if (property != null)
		{
			property.SetValue(e, true, null);
		}
	}
