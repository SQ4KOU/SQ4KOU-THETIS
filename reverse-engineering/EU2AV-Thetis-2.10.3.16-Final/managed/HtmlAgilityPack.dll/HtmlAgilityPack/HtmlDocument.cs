...
public class HtmlDocument : IXPathNavigable
{
	...
	{
		foreach (HtmlNode item in DocumentNode.SelectNodes("//" + tagName))
		{
	...
	public HtmlNode GetElementbyId(string id)
	{
		...
		if (!Nodesid.TryGetValue(id, out var value))
		{
			return null;
		}
		return value;
	...
	private void Parse()
	{
		...
		while (_index < Text.Length)
		{
			...
			case ParseState.PcDataComment:
				if (_c == 62 && _index < Text.Length && Text[_index - 3] == ']' && Text[_index - 2] == ']')
				{
					if (!PushNodeEnd(_index, close: false))
					{
						_index = Text.Length;
	...
	private bool PushNodeEnd(int index, bool close)
	{
		...
		{
			if (OptionStopperNodeName != null && _remainder == null && string.Equals(_currentnode.Name, OptionStopperNodeName, StringComparison.OrdinalIgnoreCase))
			{
	...
	private void ReadDocumentEncoding(HtmlNode node)
	{
		...
		{
			if (!string.Equals(htmlAttribute.Value, "content-type", StringComparison.OrdinalIgnoreCase))
			{
				return;
			}
