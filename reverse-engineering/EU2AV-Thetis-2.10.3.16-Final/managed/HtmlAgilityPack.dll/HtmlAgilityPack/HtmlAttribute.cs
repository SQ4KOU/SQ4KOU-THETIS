...
public class HtmlAttribute : IComparable
{
	...
	public AttributeValueQuote QuoteType
	{
		get
		{
			return _quoteType ?? InternalQuoteType ?? OwnerDocument.GlobalAttributeValueQuote ?? AttributeValueQuote.DoubleQuote;
		}
		set
		{
			_quoteType = ((value != AttributeValueQuote.Initial) ? new AttributeValueQuote?(value) : ((AttributeValueQuote?)null));
		}
	}
	...
	public string Value
	{
		...
		set
		{
			...
			{
				InternalQuoteType = ((OwnerDocument.GlobalAttributeValueQuote == AttributeValueQuote.Initial) ? AttributeValueQuote.DoubleQuote : (OwnerDocument.GlobalAttributeValueQuote ?? AttributeValueQuote.DoubleQuote));
			}
			if (_ownernode != null)
			{
				_ownernode.SetChanged();
