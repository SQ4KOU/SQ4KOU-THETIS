...
public class ModalComponentBuilder
{
	...
	public ModalComponentBuilder WithTextInput(TextInputBuilder text, int row = 0)
	{
		...
		if (_actionRows == null)
		{
			_actionRows = new List<ActionRowBuilder> { new ActionRowBuilder().AddComponent(text) };
		}
		...
		else
		{
			...
			if (actionRowBuilder.CanTakeComponent(text))
			{
				actionRowBuilder.AddComponent(text);
			}
			else
			{
				if (row >= 5)
