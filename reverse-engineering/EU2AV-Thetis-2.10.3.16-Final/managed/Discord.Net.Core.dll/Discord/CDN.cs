...
public static class CDN
{
	...
	public static string GetGuildUserBannerUrl(ulong userId, ulong guildId, string bannerId, ushort size, ImageFormat format)
	{
		...
		return string.Format("{0}guilds/{1}/users/{2}/banners/{3}.{4}?size={5}", "https://cdn.discordapp.com/", guildId, userId, bannerId, text, size);
	}
	...
	{
		return string.Format("{0}embed/avatars/{1}.png", "https://cdn.discordapp.com/", (userId >> 22) % 6);
	}
	...
	public static string GetGuildIconUrl(ulong guildId, string iconId, ushort size = 2048, ImageFormat format = ImageFormat.Auto)
	{
		...
		string text = FormatToExtension(format, iconId);
		return string.Format("{0}icons/{1}/{2}.{3}?size={4}", "https://cdn.discordapp.com/", guildId, iconId, text, size);
	}
	...
	public static string GetGuildSplashUrl(ulong guildId, string splashId, ushort size = 2048)
	{
		...
		return string.Format("{0}splashes/{1}/{2}.jpg?size={3}", "https://cdn.discordapp.com/", guildId, splashId, size);
	}
	...
	public static string GetEventCoverImageUrl(ulong guildId, ulong eventId, string assetId, ImageFormat format = ImageFormat.Auto, ushort size = 1024)
	{
		...
		{
			return string.Format("{0}guild-events/{1}/{2}.{3}?size={4}", "https://cdn.discordapp.com/", eventId, assetId, FormatToExtension(format, assetId), size);
		}
		return null;
	}
	...
	{
		return "https://cdn.discordapp.com/avatar-decoration-presets/" + avatarDecorationHash + ".png";
	}

	public static string GetGuildTagBadgeUrl(ulong guildId, string badgeHash)
	{
		return string.Format("{0}guild-tag-badges/{1}/{2}.png", "https://cdn.discordapp.com/", guildId, badgeHash);
	}
}
