...
public class Embed : IEmbed
{
	...
	internal Embed(EmbedType type, string title, string description, string url, DateTimeOffset? timestamp, Color? color, EmbedImage? image, EmbedVideo? video, EmbedAuthor? author, EmbedFooter? footer, EmbedProvider? provider, EmbedThumbnail? thumbnail, ImmutableArray<EmbedField> fields)
	{
		Type = type;
		Title = title;
	...
	{
		int num = 17;
		num = num * 23 + ((object)(Type, Title, Description, Timestamp, Color, Image, Video, Author, Footer, Provider, Thumbnail)/*cast due to constrained. prefix*/).GetHashCode();
		foreach (EmbedField field in Fields)
		{
			num = num * 23 + field.GetHashCode();
		}
		return num;
	}
}
