...
public class BucketId : IEquatable<BucketId>
{
	...
	public override int GetHashCode()
	{
		if (!IsHashBucket)
		{
			return ((object)(HttpMethod, Endpoint)/*cast due to constrained. prefix*/).GetHashCode();
		}
		return ((object)(BucketHash, string.Join("/", MajorParameters.Select((KeyValuePair<string, string> x) => x.Value)))/*cast due to constrained. prefix*/).GetHashCode();
	}
