...
internal static class ModuleClassBuilder
{
	...
	public static async Task<IEnumerable<TypeInfo>> SearchAsync(Assembly assembly, InteractionService commandService)
	{
		...
		static bool IsLoadableModule(TypeInfo info)
		{
			if (!info.IsAbstract)
			{
				return info.DeclaredMethods.SelectMany((MethodInfo x) => x.GetCustomAttributes()).Any((Attribute x) => (x is SlashCommandAttribute || x is ComponentInteractionAttribute || x is ContextCommandAttribute || x is AutocompleteCommandAttribute || x is ModalInteractionAttribute) ? true : false);
			}
			return false;
		}
	}
	...
	public static async Task<Dictionary<Type, ModuleInfo>> BuildAsync(IEnumerable<TypeInfo> validTypes, InteractionService commandService, IServiceProvider services)
	{
		...
		await commandService._cmdLogger.DebugAsync($"Successfully built {list.Count} interaction modules.").ConfigureAwait(continueOnCapturedContext: false);
		return result;
	}
	...
	private static void BuildModule(ModuleBuilder builder, TypeInfo typeInfo, InteractionService commandService, IServiceProvider services)
	{
					...
					if (!(item is EnabledInDmAttribute enabledInDmAttribute))
					{
						if (!(item is DefaultMemberPermissionsAttribute defaultMemberPermissionsAttribute))
						{
							if (!(item is PreconditionAttribute preconditionAttribute))
							{
								...
								{
									if (!(item is NsfwCommandAttribute nsfwCommandAttribute))
									{
										if (!(item is CommandContextTypeAttribute commandContextTypeAttribute))
										{
											if (item is IntegrationTypeAttribute integrationTypeAttribute)
											{
												builder.WithIntegrationTypes(integrationTypeAttribute.IntegrationTypes?.ToArray());
												continue;
											}
										...
										{
											builder.WithContextTypes(commandContextTypeAttribute.ContextTypes?.ToArray());
										}
									}
									else
									{
										builder.SetNsfw(nsfwCommandAttribute.IsNsfw);
									}
								}
								else
								{
		...
		IEnumerable<MethodInfo> enumerable5 = methods.Where(IsValidModalCommandDefinition);
	...
	private static void BuildComponentCommand(ComponentCommandBuilder builder, Func<IServiceProvider, IInteractionModuleBase> createInstance, MethodInfo methodInfo, InteractionService commandService, IServiceProvider services)
	{
		...
		foreach (Attribute item in customAttributes)
		{
			...
			else
			{
				...
				builder.TreatNameAsRegex = componentInteractionAttribute.TreatAsRegex;
			}
		}
		ParameterInfo[] parameters = methodInfo.GetParameters();
		int wildCardCount = RegexUtils.GetWildCardCount(builder.Name, commandService._wildCardExp);
	...
	private static void BuildModalCommand(ModalCommandBuilder builder, Func<IServiceProvider, IInteractionModuleBase> createInstance, MethodInfo methodInfo, InteractionService commandService, IServiceProvider services)
	{
		...
		foreach (Attribute item in customAttributes)
		{
			...
			else
			{
				...
				builder.TreatNameAsRegex = modalInteractionAttribute.TreatAsRegex;
			}
		}
		ParameterInfo[] array = parameters;
		foreach (ParameterInfo parameter in array)
		...
		async Task<IResult> ExecuteCallback(IInteractionContext context, object[] args, IServiceProvider serviceProvider, ICommandInfo commandInfo)
		{
			...
			try
			{
				try
				{
					...
					Task task = commandInvoker(instance, args) ?? Task.CompletedTask;
				...
				{
					InteractionException interactionException = new InteractionException(commandInfo, context, exception);
					await commandService._cmdLogger.ErrorAsync(interactionException).ConfigureAwait(continueOnCapturedContext: false);
					result = ExecuteResult.FromError(interactionException);
				}
				num = 1;
			}
			catch (object obj2)
	...
	private static void BuildSlashParameter(SlashCommandParameterBuilder builder, ParameterInfo paramInfo, IServiceProvider services)
	{
		...
		(double, double) supportedNumericalRange = paramInfo.GetSupportedNumericalRange();
		builder.MinValue = supportedNumericalRange.Item1;
		builder.MaxValue = supportedNumericalRange.Item2;
		foreach (Attribute item in customAttributes)
		{
							...
							if (!(item is ChannelTypesAttribute channelTypesAttribute))
							{
								if (!(item is AutocompleteAttribute autocompleteAttribute))
								{
									if (!(item is MaxValueAttribute maxValueAttribute))
									{
										...
										{
											if (minValueAttribute.Value < supportedNumericalRange.Item1)
											{
												throw new ArgumentOutOfRangeException(string.Format("{0} cannot be less than {1}.", "minValue", supportedNumericalRange.Item1));
											}
									...
									{
										if (maxValueAttribute.Value > supportedNumericalRange.Item2)
										{
											throw new ArgumentOutOfRangeException(string.Format("{0} cannot be greater than {1}.", "maxValue", supportedNumericalRange.Item2));
										}
