...
public class ModuleBuilder
{
	...
	{
		return AddAutocompleteCommand(name, callback, configure);
	}
	...
	{
		ModalCommandBuilder modalCommandBuilder = new ModalCommandBuilder(this, name, callback);
		configure(modalCommandBuilder);
		_modalCommands.Add(modalCommandBuilder);
		return this;
	}
	...
	internal ModuleInfo Build(InteractionService interactionService, IServiceProvider services, ModuleInfo parent = null)
	{
		...
		{
			using (IServiceScope serviceScope = (InteractionService._autoServiceScopes ? services.CreateScope() : null))
			{
				services = ((!InteractionService._autoServiceScopes) ? services : serviceScope?.ServiceProvider) ?? EmptyServiceProvider.Instance;
