...
public sealed class AutocompleteCommandInfo : CommandInfo<CommandParameterInfo>
{
	...
	public override Task<IResult> ExecuteAsync(IInteractionContext context, IServiceProvider services)
	{
		...
		{
			return Task.FromResult((IResult)ExecuteResult.FromError(InteractionCommandError.ParseFailed, "Provided IInteractionContext doesn't belong to a Autocomplete Interaction"));
		}
	...
	{
		return Task.FromResult((IResult)ParseResult.FromSuccess(Array.Empty<object>()));
	}
