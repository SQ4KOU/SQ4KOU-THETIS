...
internal class CommandMapNode<T> where T : class, ICommandInfo
{
	...
	public void AddCommand(IList<string> keywords, int index, T commandInfo)
	{
		...
		{
			if (commandInfo.SupportsWildCards && RegexUtils.TryBuildRegexPattern(commandInfo, _wildCardStr, out var pattern))
			{
				Regex key = new Regex(pattern, RegexOptions.Compiled | RegexOptions.Singleline);
