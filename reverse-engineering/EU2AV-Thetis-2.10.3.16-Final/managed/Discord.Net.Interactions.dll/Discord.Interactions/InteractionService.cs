...
public class InteractionService : IDisposable
{
	...
	public async Task<IReadOnlyCollection<RestGuildCommand>> RegisterCommandsToGuildAsync(ulong guildId, bool deleteMissing = true)
	{
		...
		if (!deleteMissing)
		{
			IEnumerable<RestGuildCommand> source2 = (await RestClient.GetGuildApplicationCommands(guildId, withLocalizations: true).ConfigureAwait(continueOnCapturedContext: false)).Where((RestGuildCommand x) => !props.Any((ApplicationCommandProperties y) => y.Name.IsSpecified && y.Name.Value == x.Name));
	...
	public async Task<IReadOnlyCollection<RestGlobalCommand>> RegisterCommandsGloballyAsync(bool deleteMissing = true)
	{
		...
		if (!deleteMissing)
		{
			IEnumerable<RestGlobalCommand> source2 = (await RestClient.GetGlobalApplicationCommands(withLocalizations: true).ConfigureAwait(continueOnCapturedContext: false)).Where((RestGlobalCommand x) => !props.Any((ApplicationCommandProperties y) => y.Name.IsSpecified && y.Name.Value == x.Name));
	...
	public async Task<IReadOnlyCollection<RestGuildCommand>> AddCommandsToGuildAsync(ulong guildId, bool deleteMissing = false, params ICommandInfo[] commands)
	{
		...
		if (!deleteMissing)
		{
			IEnumerable<RestGuildCommand> source = (await RestClient.GetGuildApplicationCommands(guildId, withLocalizations: true).ConfigureAwait(continueOnCapturedContext: false)).Where((RestGuildCommand x) => !props.Any((ApplicationCommandProperties y) => y.Name.IsSpecified && y.Name.Value == x.Name));
	...
	{
		EnsureClientReady();
		List<ApplicationCommandProperties> props = modules.SelectMany((ModuleInfo x) => x.ToApplicationCommandProps(ignoreDontRegister: true)).Distinct().ToList();
		if (!deleteMissing)
		{
			IEnumerable<RestGuildCommand> source = (await RestClient.GetGuildApplicationCommands(guildId, withLocalizations: true).ConfigureAwait(continueOnCapturedContext: false)).Where((RestGuildCommand x) => !props.Any((ApplicationCommandProperties y) => y.Name.IsSpecified && y.Name.Value == x.Name));
			props.AddRange(source.Select((RestGuildCommand x) => x.ToApplicationCommandProps()));
	...
	public async Task<IReadOnlyCollection<RestGlobalCommand>> AddModulesGloballyAsync(bool deleteMissing = false, params ModuleInfo[] modules)
	{
		...
		if (!deleteMissing)
		{
			IEnumerable<RestGlobalCommand> source = (await RestClient.GetGlobalApplicationCommands(withLocalizations: true).ConfigureAwait(continueOnCapturedContext: false)).Where((RestGlobalCommand x) => !props.Any((ApplicationCommandProperties y) => y.Name.IsSpecified && y.Name.Value == x.Name));
	...
	{
		EnsureClientReady();
		List<ApplicationCommandProperties> exclude = modules.SelectMany((ModuleInfo x) => x.ToApplicationCommandProps(ignoreDontRegister: true)).ToList();
		IEnumerable<ApplicationCommandProperties> source = from x in await RestClient.GetGuildApplicationCommands(guildId).ConfigureAwait(continueOnCapturedContext: false)
			where !exclude.Any((ApplicationCommandProperties y) => y.Name.IsSpecified && x.Name == y.Name.Value)
			select x.ToApplicationCommandProps();
		return await RestClient.BulkOverwriteGuildCommands(source.ToArray(), guildId).ConfigureAwait(continueOnCapturedContext: false);
	}
	...
	{
		return ModifyApplicationCommandPermissionsAsync(command, guildId, permissions);
	}
