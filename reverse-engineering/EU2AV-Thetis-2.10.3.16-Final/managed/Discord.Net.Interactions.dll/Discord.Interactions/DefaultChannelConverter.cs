...
internal class DefaultChannelConverter<T> : DefaultEntityTypeConverter<T> where T : class, IChannel
{
	...
	public DefaultChannelConverter()
	{
		...
		} : (typeof(ITextChannel).IsAssignableFrom(typeFromHandle) ? new List<ChannelType> { ChannelType.Text } : (typeof(IMediaChannel).IsAssignableFrom(typeFromHandle) ? new List<ChannelType> { ChannelType.Media } : ((!typeof(IForumChannel).IsAssignableFrom(typeFromHandle)) ? null : new List<ChannelType> { ChannelType.Forum }))))))))));
	}
