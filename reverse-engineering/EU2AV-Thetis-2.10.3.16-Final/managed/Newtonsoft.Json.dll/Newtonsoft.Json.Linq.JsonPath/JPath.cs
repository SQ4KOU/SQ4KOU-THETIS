		...
		if (char.IsDigit(c) || c == '-')
		{
			...
			while (_currentIndex < _expression.Length)
			{
				...
				if (c == ' ' || c == ')')
				{
					...
					{
						bool result = double.TryParse(text, NumberStyles.Float | NumberStyles.AllowThousands, CultureInfo.InvariantCulture, out var result2);
						value = result2;
						return result;
					}
					bool result3 = long.TryParse(text, NumberStyles.Integer, CultureInfo.InvariantCulture, out var result4);
					value = result4;
					return result3;
				}
				stringBuilder.Append(c);
