...
public sealed class ImmutableSortedSet<T> : IImmutableSet<T>, IReadOnlyCollection<T>, IEnumerable<T>, IEnumerable, IReadOnlyList<T>, IList<T>, ICollection<T>, ISet<T>, IList, ICollection, IStrongEnumerable<T, ImmutableSortedSet<T>.Enumerator>
{
	[DebuggerDisplay("Count = {Count}")]
	[DebuggerTypeProxy(typeof(ImmutableSortedSetBuilderDebuggerProxy<>))]
	public sealed class Builder : IReadOnlyCollection<T>, IEnumerable<T>, IEnumerable, ISet<T>, ICollection<T>, ICollection
	{
		...
		internal Node Remove(T key, IComparer<T> comparer, out bool mutated)
		{
			...
			if (num == 0)
			{
				...
				else
				{
					...
					Node right = _right.Remove(node2._key, comparer, out var _);
