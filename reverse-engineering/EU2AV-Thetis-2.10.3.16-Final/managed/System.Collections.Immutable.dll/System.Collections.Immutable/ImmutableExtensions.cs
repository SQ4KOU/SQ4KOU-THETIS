...
{
	private sealed class ListOfTWrapper<T> : IReadOnlyList<T>, IReadOnlyCollection<T>, IEnumerable<T>, IEnumerable
	{
	...
	private sealed class FallbackWrapper<T> : IReadOnlyList<T>, IReadOnlyCollection<T>, IEnumerable<T>, IEnumerable
	{
	...
	{
		Requires.NotNull(sequence, "sequence");
		if (sequence is IReadOnlyList<T> result)
		{
			return result;
		}
