...
public static class ImmutableDictionary
{
	...
	public static ImmutableDictionary<TKey, TValue> CreateRangeWithOverwrite<TKey, TValue>(params ReadOnlySpan<KeyValuePair<TKey, TValue>> items) where TKey : notnull
	{
	...
	public static ImmutableDictionary<TKey, TValue> CreateRangeWithOverwrite<TKey, TValue>(IEqualityComparer<TKey>? keyComparer, params ReadOnlySpan<KeyValuePair<TKey, TValue>> items) where TKey : notnull
	{
	...
	public static ImmutableDictionary<TKey, TValue> ToImmutableDictionary<TSource, TKey, TValue>(this IEnumerable<TSource> source, Func<TSource, TKey> keySelector, Func<TSource, TValue> elementSelector, IEqualityComparer<TKey>? keyComparer, IEqualityComparer<TValue>? valueComparer) where TKey : notnull
	{
		...
		return ImmutableDictionary<TKey, TValue>.Empty.WithComparers(keyComparer, valueComparer).AddRange(source.Select<TSource, KeyValuePair<TKey, TValue>>((TSource element) => new KeyValuePair<TKey, TValue>(keySelector(element), elementSelector(element))));
	}
...
[CollectionBuilder(typeof(ImmutableDictionary), "CreateRangeWithOverwrite")]
public sealed class ImmutableDictionary<TKey, TValue> : IImmutableDictionary<TKey, TValue>, IReadOnlyDictionary<TKey, TValue>, IReadOnlyCollection<KeyValuePair<TKey, TValue>>, IEnumerable<KeyValuePair<TKey, TValue>>, IEnumerable, IImmutableDictionaryInternal<TKey, TValue>, IDictionary<TKey, TValue>, ICollection<KeyValuePair<TKey, TValue>>, IDictionary, ICollection where TKey : notnull
{
	[DebuggerDisplay("Count = {Count}")]
	...
	public sealed class Builder : IDictionary<TKey, TValue>, ICollection<KeyValuePair<TKey, TValue>>, IEnumerable<KeyValuePair<TKey, TValue>>, IEnumerable, IReadOnlyDictionary<TKey, TValue>, IReadOnlyCollection<KeyValuePair<TKey, TValue>>, IDictionary, ICollection
	{
		...
		object ICollection.SyncRoot => _003CSystem_002ECollections_002EICollection_002ESyncRoot_003Ek__BackingField ?? Interlocked.CompareExchange(ref _003CSystem_002ECollections_002EICollection_002ESyncRoot_003Ek__BackingField, new object(), null) ?? _003CSystem_002ECollections_002EICollection_002ESyncRoot_003Ek__BackingField;
	...
	{
		return AddRange(pairs, Origin, collisionBehavior).Finalize(this);
	}
	...
	public ImmutableDictionary<TKey, TValue> RemoveRange(IEnumerable<TKey> keys)
	{
		...
		foreach (TKey key in keys)
		{
			...
			{
				HashBucket newBucket = value.Remove(key, _comparers.KeyOnlyComparer, out var result);
	...
	private static MutationResult Add(TKey key, TValue value, KeyCollisionBehavior behavior, MutationInput origin)
	{
		Requires.NotNullAllowStructs(key, "key");
		int hashCode = origin.KeyComparer.GetHashCode(key);
		HashBucket newBucket = origin.Root.GetValueOrDefault(hashCode).Add(key, value, origin.KeyOnlyComparer, origin.ValueComparer, behavior, out var result);
	...
	private static MutationResult AddRange(IEnumerable<KeyValuePair<TKey, TValue>> items, MutationInput origin, KeyCollisionBehavior collisionBehavior = KeyCollisionBehavior.ThrowIfValueDifferent)
	{
		...
		foreach (KeyValuePair<TKey, TValue> item in items)
		{
			...
			HashBucket newBucket = sortedInt32KeyNode.GetValueOrDefault(hashCode).Add(item.Key, item.Value, origin.KeyOnlyComparer, origin.ValueComparer, collisionBehavior, out var result);
	...
	private static MutationResult AddRange(ReadOnlySpan<KeyValuePair<TKey, TValue>> items, MutationInput origin, KeyCollisionBehavior collisionBehavior = KeyCollisionBehavior.ThrowIfValueDifferent)
	{
		int num = 0;
		SortedInt32KeyNode<HashBucket> sortedInt32KeyNode = origin.Root;
		ReadOnlySpan<KeyValuePair<TKey, TValue>> readOnlySpan = items;
		for (int i = 0; i < readOnlySpan.Length; i++)
		{
			KeyValuePair<TKey, TValue> keyValuePair = readOnlySpan[i];
			Requires.NotNullAllowStructs(keyValuePair.Key, "Key");
			int hashCode = origin.KeyComparer.GetHashCode(keyValuePair.Key);
			HashBucket newBucket = sortedInt32KeyNode.GetValueOrDefault(hashCode).Add(keyValuePair.Key, keyValuePair.Value, origin.KeyOnlyComparer, origin.ValueComparer, collisionBehavior, out var result);
			sortedInt32KeyNode = UpdateRoot(sortedInt32KeyNode, hashCode, newBucket, origin.HashBucketComparer);
			if (result == OperationResult.SizeChanged)
			{
				num++;
	...
	private static SortedInt32KeyNode<HashBucket> UpdateRoot(SortedInt32KeyNode<HashBucket> root, int hashCode, HashBucket newBucket, IEqualityComparer<HashBucket> hashBucketComparer)
	{
		...
		{
			return root.Remove(hashCode, out replacedExistingValue);
		}
		bool mutated;
