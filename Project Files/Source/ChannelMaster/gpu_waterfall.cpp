...
static float ResolvePixel(const GPUWaterfallState& s, double p0, double p1)
{
    ...
    // Quality mode keeps the recovered GPU Waterfall behaviour: Lanczos when
    // one display pixel is narrower than ~1.5 FFT bins, peak-preserving power
    // when several bins must collapse into a single pixel.
    if (s.resamplingMode == 2 || s.resamplingMode == 3)
        return PowerToDb(peakPower);

    // Power-average fallback. A small peak component preserves narrow carriers.
