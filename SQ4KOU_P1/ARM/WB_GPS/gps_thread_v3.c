...
static u32 correct_word(u32 nominal,u32 measured) {
  ...
  q *= (s64)8796; /* round(2^40 / 125000000) */
  s64 d;
...
__attribute__((used,visibility("default"))) void *gps_thread(void *arg) {
  ...
  u32 init[4]={0,0,0,0};
  const u32 offs[4]={2,3,4,5}; /* cfg +8,+12,+16,+20 */

  wb[GPS_USED]=0; wb[GPS_SWSTAT]=1; /* thread alive */
  for(;;) {
    u32 seq=wb[PPS_SEQ];
    if(seq!=last_seq) {
      u32 c=wb[PPS_COUNT];
    ...
    for(u32 i=0;i<4;i++) {
      ...
      if(locked && nominal[i]!=0) {
        ...
        if(raw!=cor) {
          /* Do not overwrite a concurrent tune made by the original ARM. */
          u32 verify=*r;
          if(verify==raw) {
            *r=cor;
            applied[i]=cor;
