...
# TX GPU waterfall implementation deliberately reuses the already-PASS RX GPU pipeline.
# RX capture remains channels 0/1. TX gets an isolated native IQ ring on channel 2, fed
# from the same post-DSP transmitter output buffer that the native TX analyzer uses.

# -----------------------------------------------------------------------------
# 1. ChannelMaster: capture TX post-DSP complex output into native ring channel 2.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 2. Managed GPU path: enable ring 2, select TX ring on MOX/non-duplex, and resize
#    the existing RX1 GPU FFT pipeline to the TX output sample rate only while TX is
#    active. Source transitions flush only the managed accumulation state; RX code,
#    ghost-fix, renderer and 8/16-bit path remain unchanged.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 3. Display integration: the recovered managed FFT/renderer must also execute on TX.
#    CPU TX waterfall remains present as calibration reference and automatic fallback.
# -----------------------------------------------------------------------------
...
Write-Host 'TX GPU Waterfall patch applied and statically verified. RX PASS path preserved.'
